...
c = {
    # GENERAL STUFF
    'suffix': f'{model.name}_conditional_hint-8-full', # identifier for trained models and outputs
...
for i in range(c['n_blocks']):
    if i > 0:
        y_lane.append(Node(y_lane[-1],
                           HouseholderPerm,
                           {'fixed': True, 'n_reflections': c['ndim_y']},
                           name=f'perm_y_{i}'))
    ...
    x_lane.append(Node(x_lane[-1],
                       ...
                       {'c_internal': [c['hidden_layer_sizes'], c['hidden_layer_sizes']//2, c['hidden_layer_sizes']//4, c['hidden_layer_sizes']//4]},
                       name=f'hac_x_{i+1}'))
